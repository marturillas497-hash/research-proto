...
export default async function DashboardPage() {
  ...
  if (profile?.role === 'research_adviser' && profile?.status === 'pending') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] text-center p-6 bg-[#F0F0F0]">
        <div className="bg-white p-12 border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] max-w-lg relative overflow-hidden">
          ...
          <h1 className="text-5xl font-black text-[#003366] uppercase mb-6 italic tracking-tighter">Access Restricted</h1>
          <p className="text-black font-bold uppercase tracking-tight leading-tight text-sm border-l-4 border-black pl-4 py-2">
          ...
          <div className="mt-10 pt-6 border-t-4 border-black flex justify-between items-center">
            <span className="text-[10px] font-black text-slate-400 uppercase">Status: PENDING_REVIEW</span>
    ...
    if (profile?.role === 'student') {
      ...
      if (studentIds.length > 0) {
        const { data } = await supabase
          ...
          .select(`
            *,
            profiles!student_id (full_name, student_metadata!student_metadata_profile_id_fkey(lrn))
          `)
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 sm:p-10 min-h-screen bg-[#F0F0F0] selection:bg-[#FFCC00]">

      {/* HEADER SECTION */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8 border-b-8 border-black pb-8">
        ...
        <div className="flex flex-col gap-3 w-full md:w-auto">
          {profile?.role === 'student' && (
            <Link href="/submit"
              className="bg-[#FFCC00] text-black px-10 py-5 border-4 border-black font-black text-sm uppercase tracking-tighter shadow-[8px_8px_0px_0px_black] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all text-center">
              + Run New Similarity Scan
            </Link>
          )}
          {profile?.role === 'admin' && (
            <Link href="/library"
              className="bg-[#003366] text-white px-10 py-5 border-4 border-black font-black text-sm uppercase tracking-tighter shadow-[8px_8px_0px_0px_black] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        ...
        <div className="bg-white p-8 border-4 border-black shadow-[10px_10px_0px_0px_black] flex flex-col justify-between">
          ...
          <div className="flex items-center justify-between">
             <p className="text-4xl font-black text-black uppercase italic tracking-tighter">
              {profile?.status}
            </p>
      ...
      {/* ACTIVITY TABLE */}
      <div className="bg-white border-4 border-black shadow-[15px_15px_0px_0px_black] overflow-hidden">
        <div className="bg-black p-6 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="w-4 h-4 bg-[#FFCC00] rotate-45"></div>
            <h2 className="text-[#FFCC00] font-black uppercase tracking-[0.3em] text-xs">
              Recent_Dossier_Log
            </h2>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b-4 border-black bg-[#f9f9f9]">
                ...
                {profile?.role !== 'student' && (
                  <th className="p-6 font-black text-black text-[10px] uppercase tracking-widest">Researcher</th>
                )}
                ...
                <tr key={item.id} className="hover:bg-[#FFCC00]/10 transition-colors group">
                  ...
                  <td className="p-6">
                    <span className={`px-4 py-2 border-2 border-black text-[9px] font-black shadow-[4px_4px_0px_0px_black] inline-block ${
                      ...
                      item.risk_level === 'YELLOW' ? 'bg-[#FFFF00] text-black' :
                  ...
                  <td className="p-6 text-right">
                    <Link href={`/dashboard/report/${item.id}`}
                      className="inline-block px-5 py-3 border-4 border-black bg-white font-black text-[10px] uppercase shadow-[5px_5px_0px_0px_black] hover:bg-[#FFCC00] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all">
                      View Dossier
                    </Link>
                  </td>
                </tr>
          ...
          {reports.length === 0 && (
            <div className="p-24 text-center border-t-4 border-black bg-white">
              <div className="w-16 h-1 w-16 h-1 bg-slate-200 mx-auto mb-4"></div>
